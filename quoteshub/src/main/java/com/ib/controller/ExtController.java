...
public class ExtController implements EWrapper {
	...
	private IBQuotesProvider m_Provider = null;
	...
	public IBQuotesProvider GetProvider() {
		return m_Provider;
	}
	...
	public interface ICommonHandler {
		...
		public static final int REALTIMEBAR = 1;
		public static final int REALTIMETICK = 2;
		public static final int HISTORICALBAR = 3;
		public static final int HISTORICALTICK = 4;
	...
	// ----------------------------------------
	public interface IConnectionHandler extends ICommonHandler {
		void connected();
	...
	public ExtController(IConnectionHandler handler, ILogger inLogger, ILogger outLogger, IBQuotesProvider p) {
	...
	@Override
	public void updatePortfolio(Contract contract, double positionIn, double marketPrice, double marketValue,
			double averageCost, double unrealizedPNL, double realizedPNL, String account) {
		contract.exchange(contract.primaryExch());

		Position position = new Position(contract, account, positionIn, marketPrice, marketValue, averageCost,
	...
	// ---------------------------------------- Contract Details
	// ----------------------------------------
	public interface IContractDetailsHandler extends ICommonHandler {
	...
	// ----------------------------------------
	public interface ITopMktDataHandler extends ICommonHandler {
	...
	public interface IEfpHandler extends ITopMktDataHandler {
		void tickEFP(int tickType, double basisPoints, String formattedBasisPoints, double impliedFuture, int holdDays,
	...
	public interface IOptHandler extends ITopMktDataHandler {
		void tickOptionComputation(TickType tickType, double impliedVol, double delta, double optPrice,
	...
	public void reqTopMktData(Contract contract, String genericTickList, boolean snapshot, boolean regulatorySnapshot,
	...
	public void reqEfpMktData(Contract contract, String genericTickList, boolean snapshot, boolean regulatorySnapshot,
			IEfpHandler handler) {
		if (!checkConnection())
			return;
	...
	public void tickEFP(int reqId, int tickType, double basisPoints, String formattedBasisPoints, double impliedFuture,
		...
		if (handler != null) {
			handler.tickEFP(tickType, basisPoints, formattedBasisPoints, impliedFuture, holdDays, futureLastTradeDate,
	...
	public interface IDeepMktDataHandler {
		void updateMktDepth(int position, String marketMaker, DeepType operation, DeepSide side, double price,
				int size);
	}
	...
	@Override
	public void updateMktDepthL2(int reqId, int position, String marketMaker, int operation, int side, double price,
			int size) {
		IDeepMktDataHandler handler = m_deepMktDataMap.get(reqId);
		if (handler != null) {
	...
	@Override
	public void tickOptionComputation(int reqId, int tickType, double impliedVol, double delta, double optPrice,
			double pvDividend, double gamma, double vega, double theta, double undPrice) {
		IOptHandler handler = m_optionCompMap.get(reqId);
		if (handler != null) {
			handler.tickOptionComputation(TickType.get(tickType), impliedVol, delta, optPrice, pvDividend, gamma, vega,
					theta, undPrice);
		} else {
			System.out.println(String.format("not handled %s %s %s %s %s %s %s %s %s", tickType, impliedVol, delta,
					optPrice, pvDividend, gamma, vega, theta, undPrice));
			System.out.printf("ReqId: %d, IV:%f, Delta:%f, optPrice: %f, underPrice: %f\n", reqId, impliedVol, delta,
					optPrice, undPrice);
		}
		recEOM();
	}
	...
	// ----------------------------------------
	/**
	 * This interface is for receiving events for a specific order placed from the
	 * API. Compare to ILiveOrderHandler.
	 */
	public interface IOrderHandler {
		void orderState(OrderState orderState);

		void orderStatus(OrderStatus status, double filled, double remaining, double avgFillPrice, int permId,
	...
	public void orderStatus(int orderId, String status, double filled, double remaining, double avgFillPrice,
		...
		IOrderHandler handler = m_orderHandlers.get(orderId);
		if (handler != null) {
			handler.orderStatus(OrderStatus.valueOf(status), filled, remaining, avgFillPrice, permId, parentId,
					lastFillPrice, clientId, whyHeld, mktCapPrice);
		}

		for (ILiveOrderHandler liveOrderHandler : m_liveOrderHandlers) {
			liveOrderHandler.orderStatus(orderId, OrderStatus.valueOf(status), filled, remaining, avgFillPrice, permId,
	...
	// ----------------------------------------
	public interface IHistoricalDataHandler extends ICommonHandler {
	...
	public void reqHistoricalData(Contract contract, String endDateTime, int duration, DurationUnit durationUnit,
			BarSize barSize, WhatToShow whatToShow, boolean rthOnly, boolean keepUpToDate,
		...
		m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, barSize.toString(), whatToShow.toString(),
				rthOnly ? 1 : 0, 2, keepUpToDate, Collections.emptyList());
		sendEOM();
	}
	...
	public void historicalData(int reqId, com.ib.client.Bar bar) {
		...
		if (handler != null) {
			if (bar.time().startsWith("finished")) {
				...
				Bar bar2 = new Bar(longDate, bar.high(), bar.low(), bar.open(), bar.close(), bar.wap(), bar.volume(),
						bar.count());
	...
	// ----------------------------------------- Real-time bars
	// --------------------------------------
	public interface IRealTimeBarHandler extends ICommonHandler {
	...
	public void reqSecDefOptParams(String underlyingSymbol, String futFopExchange,
			/* String currency, */ String underlyingSecType, int underlyingConId, ISecDefOptParamsReqHandler handler) {
		if (!checkConnection())
			return;

		int reqId = m_reqId++;
		m_secDefOptParamsReqMap.put(reqId, handler);
		m_client.reqSecDefOptParams(reqId, underlyingSymbol, futFopExchange, /* currency, */ underlyingSecType,
				underlyingConId);
		sendEOM();
	}
	...
	public void securityDefinitionOptionalParameter(int reqId, String exchange, int underlyingConId,
		...
		if (handler != null) {
			handler.securityDefinitionOptionalParameter(exchange, underlyingConId, tradingClass, multiplier,
					expirations, strikes);
		}
	}
	...
	public void reqHistoricalNews(int conId, String providerCodes, String startDateTime, String endDateTime,
			int totalResults, IHistoricalNewsHandler handler) {
		if (!checkConnection())
			return;
	...
	public void reqHistogramData(Contract contract, int duration, DurationUnit durationUnit, boolean rthOnly,
			IHistogramDataHandler handler) {
		if (!checkConnection())
			return;
	...
	public void reqHistoricalTicks(Contract contract, String startDateTime, String endDateTime, int numberOfTicks,
			String whatToShow, int useRth, boolean ignoreSize, IHistoricalTickHandler handler) {
		if (!checkConnection())
			return;
		...
		m_client.reqHistoricalTicks(reqId, contract, startDateTime, endDateTime, numberOfTicks, whatToShow, useRth,
				ignoreSize, Collections.emptyList());
	}
	...
	public interface ITickByTickDataHandler {
		void tickByTickAllLast(int reqId, int tickType, long time, double price, int size, TickAttr attribs,
